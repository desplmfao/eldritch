 ...
 *
 * you can find this file at: https://github.com/desplmfao/eldritch/blob/main/src/engine/plugins/edict/tests/command_execution/command_execution_edge_cases.test.ts
 */
...
describe('edict - command pipeline edge cases', () => {
   let world: World;
   let command_input: ResourceRawCommandInput;
   let app_system: SystemHealPlayer;
   let heal_command_id: EntityId;
   let ban_command_id: EntityId;
   ...
   beforeEach(async () => {
      ...
      heal_command_id = await world.entity_spawn_direct({
         components: [
            [
               ComponentName, {
                  value: 'heal'
      ...
      ban_command_id = await world.entity_spawn_direct({
         components: [
            [
               ComponentName,
               {
   ...
   it('should not execute a command if the sender entity is deleted mid-pipeline', async () => {
      const admin_id = await world.entity_spawn_direct({ components: [[PermissionAdmin, {}]] });
      ...
      await world.entity_delete_direct(admin_id);
   ...
   it('should not execute a command if its definition is deleted mid-pipeline', async () => {
      ...
      await world.entity_delete_direct(heal_command_id);
   ...
   it('should not execute if sender loses permissions mid-pipeline', async () => {
      const user_id = await world.entity_spawn_direct({ components: [[PermissionAdmin, {}]] });

      command_input.queue.push({
         raw_string: 'ban another_user',
      ...
      await world.component_remove_multiple_direct(user_id, [PermissionAdmin.name]);
   ...
   it('should correctly process a mixed batch of commands with failures', async () => {
      const admin_id = await world.entity_spawn_direct({ components: [[PermissionAdmin, {}]] });
      const regular_user_id = await world.entity_create_direct();

      command_input.queue.push(
         { raw_string: 'heal notch 100', sender_entity_id: regular_user_id },     // should succeed
         { raw_string: 'ban user1', sender_entity_id: admin_id },                 // should succeed
         { raw_string: 'heal jeb bad_value', sender_entity_id: regular_user_id }, // will fail parsing
         { raw_string: 'ban user2', sender_entity_id: regular_user_id }           // will fail permissions
      );
      ...
      expect(app_system.ban_function).toHaveBeenCalledWith('user1');
   ...
   it('should gracefully ignore empty or whitespace-only command strings', async () => {
      const sender_id = await world.entity_create_direct();

      command_input.queue.push(
         { raw_string: '', sender_entity_id: sender_id },
         { raw_string: '   ', sender_entity_id: sender_id }
      );
