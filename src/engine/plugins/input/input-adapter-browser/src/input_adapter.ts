 ...
 *
 * you can find this file at: https://github.com/desplmfao/eldritch/blob/main/src/engine/plugins/input/input-adapter-browser/src/input_adapter.ts
 */
...
export class BrowserInputAdapter implements InputAdapter {
   ...
   constructor(
      canvas_element?: HTMLCanvasElement | string | null
   ) {
      const logger = default_logger.get_namespaced_logger('<namespace>');

      if (typeof canvas_element === 'string') {
         this.#canvas_element = document.querySelector(canvas_element) as HTMLCanvasElement | undefined;

         if (!this.#canvas_element) {
            logger.warn(`canvas element not found for selector: ${canvas_element}`);
         }
      } else if (canvas_element instanceof HTMLCanvasElement) {
         this.#canvas_element = canvas_element;
      } else {
         this.#canvas_element = document.querySelector('#render-canvas') as HTMLCanvasElement | undefined;

         if (!this.#canvas_element) {
            logger.warn('canvas element not provided and #render-canvas not found');
         }
      }
   ...
   async initialize(): Promise<void> {
      ...
      logger.trace('initialized all input handlers');
   }
   ...
   async cleanup(): Promise<void> {
      ...
      logger.trace('removed all input handlers');
   }
