 ...
 *
 * you can find this file at: https://github.com/desplmfao/eldritch/blob/main/src/engine/packages/ecs/ecs-core/src/ecs/systems/events/relationship/child_of_added.ts
 */
...
export class EventOnRelationshipChildOfAdded extends WorldEventHandler<'component_added'> {
   async update(
      ...
      if (!(added_component_generic instanceof ComponentChildOf)) {
         return;
      }

      const added_component = added_component_generic as ComponentChildOf;
      const old_relationship_state = context?.overwritten_component as ComponentChildOf | undefined;

      const metadata = relationship_get_metadata(relationship_registry, ComponentChildOf.name);

      if (!metadata) {
         logger.warn(`entity ${source_entity_id}: handling add for ${ComponentChildOf.name}, but no metadata found`);
      ...
      const handler_context: RelationshipHandlerContext = {
         ...
         notifications: {
            source_added_to_target_event_name: 'entity_child_added',
