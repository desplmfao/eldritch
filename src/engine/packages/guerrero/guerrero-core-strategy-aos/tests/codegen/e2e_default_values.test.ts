 ...
 *
 * you can find this file at: https://github.com/desplmfao/eldritch/blob/main/src/engine/packages/guerrero/guerrero-core-strategy-aos/tests/codegen/e2e_default_values.test.ts
 */
...
describe('codegen e2e - default values', () => {
   const source_code = `\
...
export class ComponentWithDefaults {
   ...
   string_array!: t<str[]> = ['hello', 'world'];

   // these require the decorator version because its not possible to use the object as the NestedStruct without a type assertion
...
`;
   ...
   beforeEach(async () => {
      ...
      temp_out_dir = await fs.mkdtemp(path.join(os.tmpdir(), 'eldritch-test-codegen-defaults-'));
      ...
      component = new GeneratedComponent(allocator.buffer, ptr, allocator, undefined, undefined, true);
   });
