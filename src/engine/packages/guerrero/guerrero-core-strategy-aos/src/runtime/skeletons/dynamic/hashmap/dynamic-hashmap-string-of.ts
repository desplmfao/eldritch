 ...
 *
 * you can find this file at: https://github.com/desplmfao/eldritch/blob/main/src/engine/packages/guerrero/guerrero-core-strategy-aos/src/runtime/skeletons/dynamic/hashmap/dynamic-hashmap-string-of.ts
 */
...
export class DynamicHashMapStringOf<V extends IView & { $copy_from(source: V): void; free?(): void; }> extends DynamicHashMapString<V, V> {
   ...
   constructor(
      ...
      /// #if SAFETY
      if ((byte_offset % DynamicHashMapStringOf.__schema.alignment) !== 0) {
         throw new Error(`${DynamicHashMapStringOf.__schema.class_ctor?.name} byte_offset ${byte_offset} is not aligned to ${DynamicHashMapStringOf.__schema.alignment}`);
      }
      /// #endif
   ...
   $get_internal_value_view_from_node(
      ...
      const value_offset_in_node = this.$entry_node_value_offset;
      const value_offset_in_pool = node_ptr + value_offset_in_node;
