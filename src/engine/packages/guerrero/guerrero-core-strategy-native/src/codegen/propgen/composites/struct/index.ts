 ...
 *
 * you can find this file at: https://github.com/desplmfao/eldritch/blob/main/src/engine/packages/guerrero/guerrero-core-strategy-native/src/codegen/propgen/composites/struct/index.ts
 */
...
export class StructPropgen implements IPropertyCodegen {
   ...
   generate_private_view_field(
      ...
      const type_name = property_layout.type;
...
`;
   ...
   generate_getter(
      ...
      const is_optional = property_layout.binary_info.is_optional;
      const type_name = property_layout.type;
      const return_type = is_optional ? `${type_name} | undefined` : type_name;
...
`;
   ...
   generate_setter(
      ...
      const is_optional = property_layout.binary_info.is_optional;
      const type_name = property_layout.type;
      const parameter_setter = is_optional ? `value?: ${type_name}` : `value: ${type_name}`;

      return `\
   ${setter_access} ${prop_key_str}(${parameter_setter}) {
